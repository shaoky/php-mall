...
class GoodsSpec extends Model {
    ...
    public function add($params) {
        try {
            $data = Db::name('goods_spec')->insert($params);
            return '添加成功';
        } catch (\Exception $e) {
    ...
    public function getList($params) {
        $data['list'] = Db::name('goods_spec')->where('gbId', $params['gbId'])->order('specSort', 'desc')->page($params['page'], $params['size'])->select();
        foreach($data['list'] as &$item) {
            $item['specTypeName'] = getStatusName('specType', $item['specType']);
            $item['isOpen'] = getStatusName('isOpen', $item['isOpen']);
        }
        $data['count'] = Db::name('goods_spec')->count();
    ...
    public function updateGoodsSpec($params) {
        try {
            $data = Db::name('goods_spec')->where('specId', $params['specId'])->update($params);
            if ($data == 1) {
                return '更新成功';
            } else {
                $this->error = '更新失败';
    ...
    public function deleteGoodsSpec($params) {
        ...
        Db::startTrans();
        try {
            $data = Db::name('goods_spec')->where('gbId', $params['gbId'])->delete();
    ...
    public function getGoodsSpecInfo($params) {
        if (empty($params['gbId'])) {
            $this->error = '类型id出错了';
            return false;
        }
        try {
            $data['info'] = Db::name('goods_spec')->where('gbId', $params['gbId'])->find();
            return $data;
        } catch (\Exception $e) {
            $this->error = $e->getMessage();
            return false;
